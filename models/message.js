...
onMessage((message) => {
    ...
    if (data.type == 'FriendMessage')
        isFriendMessage(data);
});
// 处理函数
// 是群消息
const isGroupMessage = (data) => {
...
const isBotCommand = (str, value = 1) => {
...
export const sendTextMsg = (type, ID, msg) => {
    if (type == 'group') {
...
// 命令错误发送错误信息
export const sendCmdErrMsg = (type, ID) => {
    if (type == 'group') {
...
export const sendGroupTextMessage = (groupID, message) => {
    sendGroupMessage(groupID, [{ type: 'Plain', text: message }]);
};
// 发送好友文字信息 sendFriendTextMessage(好友QQ号,消息内容)
export const sendFriendTextMessage = (friendID, message) => {
